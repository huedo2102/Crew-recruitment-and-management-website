...
<div>
        ...
        <table id="dataTable">
            ...
            foreach ($list as $key => $value) { ?>
                <tr data-id="<?=$value['id_thuyenvien']?>" onclick="showDetail(this)">
                    ...
                    <td><?=$value['tentau']?></td>
                    <td><?=formatDate($value['ngaybatdau'])?></td>
                    <td><?=$value['thoihan']?></td>
    ...
    <h3 id="chitiet_thuyenvien">Thông tin chi tiết</h3>
    <div>
        <div class="tab">
...
<script>
    ...
    function showDetail(row) {
        ...
        var thuyenvienName = "Thông tin chi tiết của TV:  <em style='font-style: italic; color: #888'>" 
                + detail.tenthuyenvien +" - Mã TV: "+ detail.id_thuyenvien + "</em>";
                document.getElementById("chitiet_thuyenvien").innerHTML = thuyenvienName;
        ...
        if (selectedTruongHoc.length > 0) {
            ...
            selectedTruongHoc.forEach(function(item) {
                ...
                row.innerHTML = `
                    ...
                    <td>${formatDate(item.batdau)}</td>
                    <td>${formatDate(item.ketthuc)}</td>
                    <td>${item.xeploai}</td>
                    <td>${item.ghichu}</td>
                    <td>
                        <div style="display: flex;">
